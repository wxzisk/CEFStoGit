...
class _VariableStore(object):
  ...
  def get_variable(self,
    ...
    """
    if trainable:
      if os.environ.get("PARTITIONER") is not None:
        num_partitioners = int(os.environ.get("PARTITIONER"))
      else:
        num_partitioners = 1
      if os.environ.get("SLICING_THRESHOLD") is not None:
        slicing_threshold = int(os.environ.get("SLICING_THRESHOLD"))
      else:
        slicing_threshold = 1

      if num_partitioners != 1 or slicing_threshold != 1:
        if shape:
          if slicing_threshold != 1:
            slices = reduce(lambda x, y: x * y, shape) // num_partitioners // slicing_threshold
            if slices == 0:
              slices = 1
          else:
            slices = 1
          slices = slices * num_partitioners
          if partitioner is None and slices > 1:
            def fixed_size_partitioner(num_shards, axis=0):
              ...
              return _partitioner
            #partitioner = fixed_size_partitioner(slices, shape.index(max(shape)))
