...
Status GrpcServer::WorkerCacheFactory(const WorkerCacheFactoryOptions& options,
  ...
  for (auto iter : channel_spec.host_ports_jobs()) {
    for (auto p : iter.host_ports) {
      ip_table_.insert(std::make_pair(std::make_pair(iter.job_id, p.first), 
          str_util::Split(p.second, ':')[0]));
    }
  }
