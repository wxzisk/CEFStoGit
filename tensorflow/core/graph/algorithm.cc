...
template <typename T>
void ReverseBFSFromHelper(const Graph& g, gtl::ArraySlice<T> start,
                          const std::function<void(T)>& enter,
                          const std::function<void(T)>& leave,
                          const NodeComparator& stable_comparator) {
  // Stack of work to do.
  ...
  while (!stack.empty()) {
    ...
    // Arrange to call leave(n) when all done with descendants.
    if (leave) stack.insert(stack.begin(), Work{n, true});
...
void ReverseBFSFrom(const Graph& g, gtl::ArraySlice<const Node*> start,
                    const std::function<void(const Node*)>& enter,
                    const std::function<void(const Node*)>& leave,
                    const NodeComparator& stable_comparator) {
  ReverseBFSFromHelper(g, start, enter, leave, stable_comparator);
}

void ReverseBFSFrom(const Graph& g, gtl::ArraySlice<Node*> start,
                    const std::function<void(Node*)>& enter,
                    const std::function<void(Node*)>& leave,
                    const NodeComparator& stable_comparator) {
  ReverseBFSFromHelper(g, start, enter, leave, stable_comparator);
