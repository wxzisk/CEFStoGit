...
ibv_device* set_device() {
  ...
  string str_flow_priority = get_env_var("FLOW_PRIORITY");
  if (str_flow_priority.empty())
    flow_priority = 0;
  else
  {
    priority_vec = split_and_convert_uint8(str_flow_priority, ',');
    std::sort(priority_vec.begin(), priority_vec.end());
    reverse(priority_vec.begin(), priority_vec.end());
    flow_priority = priority_vec.size();
  }
...
RdmaChannel::RdmaChannel(const RdmaAdapter* adapter, const string local_name,
  ...
  // Local address
  {
    ...
    RDMA_LOG(0) << "remote_name : " << remote_name_ << " with queue pair number " << self_.qpn;
...
void RdmaChannel::Connect(const string& priority) {
  {
    mutex_lock lock{mu_};
    CHECK(remote_set_) << "remote channel is not set";
  }
  Connect(remote_, priority);
}
...
// Returns:
//   None
void RdmaChannel::Connect(const RdmaAddress& remoteAddr, const string& priority) {
  mutex_lock lock{mu_};
  if (!connected_) {
    struct ibv_qp_attr attr;
    memset(&attr, 0, sizeof(ibv_qp_attr));
    ...
    if (priority.empty())
      ...
      attr.ah_attr.sl = (uint8_t)stoi(priority);
    ...
    if (priority.empty())
      ...
      attr.ah_attr.grh.traffic_class = (uint8_t)stoi(priority);
...
void RdmaTensorRequest::RecvTensorContent() {
  ...
  // collect disorder index
  // VLOG(INFO) << "Step " << step_id_ << ": " << key_;

  if (should_profile_) {
    struct timespec tv;
    string simple_edge_name;
    bool success;
    decltype(edges_set_)::iterator iter;

    clock_gettime(CLOCK_REALTIME, &tv);
    RE2::PartialMatch(key_, "edge_\\d+_(.+);", &simple_edge_name);
    std::tie(iter, success) = edges_set_.insert(std::make_pair(simple_edge_name, true));
    if (success && gradient_table_.find(simple_edge_name) != gradient_table_.end()) {
      worker_power_vector_.clear();
      last_egde_name = simple_edge_name;
      int task_id;
      RE2::PartialMatch(key_, "job:worker/replica:\\d+/task:(\\d+)/", &task_id);
      size_t tmp_time = (tv.tv_sec - ccp_start_tv_.tv_sec) * 1e3 + (tv.tv_nsec - ccp_start_tv_.tv_nsec) / 1e6;
      worker_power_vector_[std::to_string(task_id)].push_back(tmp_time);
    }
    else if (!success && last_egde_name == simple_edge_name) {
      int task_id;
      RE2::PartialMatch(key_, "job:worker/replica:\\d+/task:(\\d+)/", &task_id);
      size_t tmp_time = (tv.tv_sec - ccp_start_tv_.tv_sec) * 1e3 + (tv.tv_nsec - ccp_start_tv_.tv_nsec) / 1e6;
      worker_power_vector_[std::to_string(task_id)].push_back(tmp_time);
